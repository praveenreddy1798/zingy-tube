...
export const getVideoHandler = function (schema, request) {
  ...
  try {
    const video = schema.videos.findBy({ id: videoId }).attrs;
    const mustWatchVideos = this.db.videos.filter(
      (video) => video.videoType === "Must watch" && video.id !== videoId
    );
    return new Response(200, {}, { video, mustWatchVideos });
  } catch (error) {
    return new Response(
      500,
      {},
